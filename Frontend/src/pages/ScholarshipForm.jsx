...
const FileUpload = ({ label }) => (
  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 py-2 border-b border-gray-100">
    ...
    <label className="cursor-pointer self-start sm:self-auto flex-shrink-0">
      ...
      <span className="border border-gray-400 text-xs px-4 py-1 rounded hover:bg-gray-50 transition-colors block">
        Upload
      </span>
    </label>
  </div>
...
export default function ScholarshipForm() {
  ...
  useEffect(() => {
    const schemeMap = {
      ...
      pragati: "Pragati Scholarship",
      ntse: "NTSE",
      nms: "National Merit Scholarship",
      css: "Central Scholarship Scheme",
    };

    // Pre-fill Scheme based on URL parameter
    if (schemeId && schemeMap[schemeId]) {
      setForm((f) => ({ ...f, scheme: schemeMap[schemeId] }));
    }

    // Fetch Student data to pre-fill the form seamlessly
    fetch("/api/auth/me", { credentials: "include" })
      ...
      .then((data) => {
        ...
        if (s) {
          setForm((f) => ({
            ...f,
            aadhar: s.aadhar || f.aadhar,
            instituteCode: s.instituteCode || f.instituteCode,
            contactState: s.state || f.contactState,
            contactDistrict: s.district || f.contactDistrict,
          }));
          if (s.instituteCode) {
            fetch(`/api/auth/institute/info/${s.instituteCode}`, {
              credentials: "include",
            })
              .then((r) => r.json())
              .then((instData) => {
              if (instData.name) {
                  setForm((f) => ({ ...f, instituteName: instData.name }));
                setErrors((e) => ({ ...e, instituteName: "" }));
              }
              })
              .catch(console.error);
          }
  ...
  const handleInstituteCodeBlur = async () => {
    ...
    try {
      const res = await fetch(
        `/api/auth/institute/info/${form.instituteCode}`,
        { credentials: "include" },
      );

      if (!res.headers.get("content-type")?.includes("application/json")) {
        throw new Error(
          "Invalid server response. Please ensure the backend is running and updated.",
        );
      }
      ...
      if (res.ok) {
            setForm((f) => ({ ...f, instituteName: data.name }));
            setErrors((e) => ({ ...e, instituteName: "" }));
      } else {
            setForm((f) => ({ ...f, instituteName: "" }));
            setErrors((e) => ({ ...e, instituteName: data.error || "Institute not found" }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        `Application Submitted Successfully! Your Application ID is: ${data.appId}`,
      );
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={form.aadhar}
                    onChange={(e) =>
                      handleFieldChange("aadhar", e.target.value, "digits")
                    }
                  />
                  {errors.aadhar && (
                ...
                <div>
                  ...
                  <select
                    className={`form-select ${errors.religion ? "border-red-500" : ""}`}
                    value={form.religion}
                    onChange={(e) =>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    className={`form-input ${errors.instituteCode ? "border-red-500" : ""}`}
                    placeholder="e.g. NSP001"
                    value={form.instituteCode}
                    onChange={(e) =>
                      handleFieldChange("instituteCode", e.target.value, "uppercase")
                    }
                    onBlur={handleInstituteCodeBlur}
                  />
                ...
                <div>
                  ...
                  <input
                    className={`form-input bg-gray-100 cursor-not-allowed ${errors.instituteName ? "border-red-500" : ""}`}
                ...
                <div>
                  ...
                  <select
                    className={`form-select ${errors.classYear ? "border-red-500" : ""}`}
                    value={form.classYear}
                    onChange={(e) =>
                      handleFieldChange("classYear", e.target.value)
                    }
                  >
                    <option value="">-- Select --</option>
                    {form.presentClass && !isNaN(form.presentClass) ? (
                      <option value={form.presentClass}>
                        {form.presentClass}
                      </option>
                    ) : (
                      ["1", "2", "3", "4", "5"].map((y) => (
                ...
                <div>
                  ...
                  <select
                    className={`form-select ${errors.modeOfStudy ? "border-red-500" : ""}`}
                    value={form.modeOfStudy}
                    onChange={(e) =>
                      handleFieldChange("modeOfStudy", e.target.value)
                    }
                  >
                    <option value="">-- Select --</option>
                ...
                <div>
                  ...
                  <input
                    type="date"
                    className={`form-input ${errors.classStartDate ? "border-red-500" : ""}`}
                    value={form.classStartDate}
                    onChange={(e) =>
                      handleFieldChange("classStartDate", e.target.value)
                    }
                  />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleFieldChange("prevClass", e.target.value)
                    }
                  >
                    <option value="">-- Select --</option>
                    {courses.map((c) => (
                ...
                <div>
                  ...
                  <select
                    className="form-select"
                    value={form.prevPassYear}
                    onChange={(e) =>
                      handleFieldChange("prevPassYear", e.target.value)
                    }
                  >
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className={`form-input ${errors.prevClassPercent ? "border-red-500" : ""}`}
                    min="0"
                    max="99"
                    step="0.01"
                    value={form.prevClassPercent}
                    onChange={(e) =>
                      handleFieldChange("prevClassPercent", e.target.value)
                    }
                  />
                  {errors.prevClassPercent && (
                    <div className="text-xs text-red-600 mt-1">
                      {errors.prevClassPercent}
                    </div>
                  )}
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    className={`form-input ${errors.roll10 ? "border-red-500" : ""}`}
                    maxLength={8}
                    value={form.roll10}
                    onChange={(e) =>
                      handleFieldChange("roll10", e.target.value, "digits")
                    }
                  />
                  {errors.roll10 && (
                    <div className="text-xs text-red-600 mt-1">
                ...
                <div>
                  ...
                  <select
                    className={`form-select ${errors.board10 ? "border-red-500" : ""}`}
                    value={form.board10}
                    onChange={(e) =>
                      handleFieldChange("board10", e.target.value)
                    }
                  >
                ...
                <div>
                  ...
                  <select
                    className={`form-select ${errors.year10 ? "border-red-500" : ""}`}
                    value={form.year10}
                    onChange={(e) =>
                      handleFieldChange("year10", e.target.value)
                    }
                  >
                    <option value="">-- Select Year --</option>
                    {years.map((y) => (
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className={`form-input ${errors.percent10 ? "border-red-500" : ""}`}
                    min="0"
                    max="99"
                    step="0.01"
                    value={form.percent10}
                    onChange={(e) =>
                      handleFieldChange("percent10", e.target.value)
                    }
                  />
                  {errors.percent10 && (
                    <div className="text-xs text-red-600 mt-1">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="form-label">Roll Number</label>
                  <input
                    className={`form-input ${errors.roll12 ? "border-red-500" : ""}`}
                    maxLength={8}
                    value={form.roll12}
                    onChange={(e) =>
                      handleFieldChange("roll12", e.target.value, "digits")
                    }
                  />
                  {errors.roll12 && (
                    <div className="text-xs text-red-600 mt-1">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={form.board12}
                    onChange={(e) =>
                      handleFieldChange("board12", e.target.value)
                    }
                  >
                    <option value="">-- Select Board --</option>
                    {boardNames.map((b) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    value={form.year12}
                    onChange={(e) =>
                      handleFieldChange("year12", e.target.value)
                    }
                  >
                    <option value="">-- Select Year --</option>
                    {years.map((y) => (
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className={`form-input ${errors.percent12 ? "border-red-500" : ""}`}
                    min="0"
                    max="99"
                    step="0.01"
                    value={form.percent12}
                    onChange={(e) =>
                      handleFieldChange("percent12", e.target.value)
                    }
                  />
                  {errors.percent12 && (
                    <div className="text-xs text-red-600 mt-1">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    className={`form-input ${errors.admissionFee ? "border-red-500" : ""}`}
                    min="0"
                    max="99999"
                    value={form.admissionFee}
                    onChange={(e) =>
                      handleFieldChange("admissionFee", e.target.value)
                    }
                  />
                  {errors.admissionFee && (
                    <div className="text-xs text-red-600 mt-1">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className={`form-input ${errors.tuitionFee ? "border-red-500" : ""}`}
                    min="0"
                    max="99999"
                    value={form.tuitionFee}
                    onChange={(e) =>
                      handleFieldChange("tuitionFee", e.target.value)
                    }
                  />
                  {errors.tuitionFee && (
                    <div className="text-xs text-red-600 mt-1">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className={`form-input ${errors.otherFee ? "border-red-500" : ""}`}
                    min="0"
                    max="99999"
                    value={form.otherFee}
                    onChange={(e) =>
                      handleFieldChange("otherFee", e.target.value)
                    }
                  />
                  {errors.otherFee && (
                    <div className="text-xs text-red-600 mt-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleFieldChange("isDisabled", e.target.value)
                    }
                  >
                    <option>No</option>
                    <option>Yes</option>
                ...
                {form.isDisabled === "Yes" && (
                  <>
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        className={`form-input ${errors.disabilityPercent ? "border-red-500" : ""}`}
                        min="0"
                        max="100"
                        step="0.01"
                        value={form.disabilityPercent}
                        onChange={(e) =>
                          handleFieldChange("disabilityPercent", e.target.value)
                        }
                      />
                    </div>
                  </>
                ...
                <div>
                  ...
                  <input
                    className="form-input"
                    maxLength={100}
                    value={form.parentsProfession}
                    onChange={(e) =>
                      handleFieldChange("parentsProfession", e.target.value)
                    }
                  />
                </div>
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    className={`form-select ${errors.contactState ? "border-red-500" : ""}`}
                    value={form.contactState}
                    onChange={(e) =>
                      handleFieldChange("contactState", e.target.value)
                    }
                  >
                    <option value="">-- Select --</option>
                    {states.map((s) => (
                ...
                <div>
                  ...
                  <select
                    className={`form-select ${errors.contactDistrict ? "border-red-500" : ""}`}
                    value={form.contactDistrict}
                    onChange={(e) =>
                      handleFieldChange("contactDistrict", e.target.value)
                    }
                  >
                    <option value="">-- Select --</option>
                    {districts.map((d) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleFieldChange("contactBlock", e.target.value)
                    }
                  >
                    <option value="">-- Select --</option>
                    {blocks.map((b) => (
                ...
                <div>
                  ...
                  <input
                    className={`form-input ${errors.houseNo ? "border-red-500" : ""}`}
                    maxLength={10}
                    value={form.houseNo}
                    onChange={(e) =>
                      handleFieldChange("houseNo", e.target.value, "uppercase")
                    }
                  />
                  {errors.houseNo && (
                    <div className="text-xs text-red-600 mt-1">
                ...
                <div>
                  ...
                  <input
                    className={`form-input ${errors.streetNo ? "border-red-500" : ""}`}
                    maxLength={2}
                    value={form.streetNo}
                    onChange={(e) =>
                      handleFieldChange("streetNo", e.target.value, "digits")
                    }
                  />
                  {errors.streetNo && (
                    <div className="text-xs text-red-600 mt-1">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={form.pincode}
                    onChange={(e) =>
                      handleFieldChange("pincode", e.target.value, "digits")
                    }
                  />
                  {errors.pincode && (
                    <div className="text-xs text-red-600 mt-1">
            ...
            <section>
              ...
              <select
                className={`form-select ${errors.scheme ? "border-red-500" : ""}`}
                value={form.scheme}
              ...
              {errors.scheme && (
                <div className="text-xs text-red-600 mt-1">{errors.scheme}</div>
              )}
            </section>
            ...
            <div className="flex items-start gap-3 bg-yellow-50 border border-yellow-200 rounded p-3">
              <input
                ...
                checked={form.agree}
                onChange={(e) => handleFieldChange("agree", e.target.checked)}
              />
              <label htmlFor="agree" className="text-xs text-gray-600">
                All the details furnished by me are true to the best of my
                knowledge. If any mistakes are found then I may be disqualified
                for scholarship scheme announced by Government of India or my
                State Government.
              </label>
              {errors.agree && (
                <div className="text-xs text-red-600 mt-1 block w-full">
                  {errors.agree}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button
                ...
                onClick={() => setRedirect("/dashboard/student")}
              >
                Cancel
              </button>
            </div>
