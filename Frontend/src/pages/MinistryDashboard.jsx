  ...
  const label =
    s === "Approved"
      ...
      : s === "MinistryPending"
        ...
        : s;
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${c}`}>
      {label}
    </span>
  );
};
  ...
  const c =
    s === "MinistryPending"
      ...
      : s === "Approved"
        ? "bg-green-100 text-green-700"
        : "bg-red-100 text-red-700";
  const label = s === "MinistryPending" ? "Forwarded to Ministry" : s;
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${c}`}>
      {label}
    </span>
...
function MinistryDashboard() {
  ...
  const loadInstituteApps = async () => {
    ...
    try {
      const res = await fetch(
        "/api/auth/officer/ministry/institute-applications",
        { credentials: "include" },
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(data.error || `Failed to load (${res.status})`);
      setInstApps((data.apps || []).map((app) => ({ ...app, id: app._id })));
    } catch (err) {
  ...
  useEffect(() => {
    if (tab === "students") {
      setLoadingInst(true);
      fetch("/api/auth/officer/ministry/scholarship-applications", {
        credentials: "include",
      })
        .then((res) => res.json())
  ...
  const handleStudentAction = async (id, action) => {
    try {
      const res = await fetch(`/api/auth/officer/scholarship/${id}/decision`, {
        ...
        credentials: "include",
        body: JSON.stringify({
          decision: action === "grant" ? "approve" : "reject",
        }),
      });
      if (!res.ok) throw new Error("Action failed");
      alert(
        action === "grant"
          ? "Scholarship granted! Status updated to Approved."
          : "Application rejected.",
      );
      // Refresh list
      const listRes = await fetch(
        "/api/auth/officer/ministry/scholarship-applications",
        { credentials: "include" },
      );
    ...
    try {
      ...
      setInstApps((prev) =>
        prev.map((a) =>
          a.id === id
            ? {
                ...a, // The status is now directly from the backend response
      ...
      alert(
        action === "approve"
          ? "Institute registration approved! Login credentials activated."
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-6 space-y-6">
        ...
        <div className="card flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500">
              Ministry of Education – National Scholarship Portal Admin
            </p>
          </div>
          <Link
            to="/"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {stats.map((s) => (
            <div
              key={s.label}
              className={`border rounded-lg p-3 sm:p-4 text-center ${s.color}`}
            >
        ...
        <div className="card">
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                <h2 className="section-title mb-0">
                  Student Applications (Approved by State)
                </h2>
                <button className="btn-primary text-xs px-4 py-1 w-full sm:w-auto">
                  Fetch Forms
                </button>
              </div>
                  ...
                  <div key={app._id} className="border rounded p-3 bg-white">
                    ...
                    {app.status === "MinistryPending" && (
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={() => handleStudentAction(app._id, "grant")}
                          className="flex-1 text-xs bg-green-600 text-white py-1.5 rounded hover:bg-green-700"
                        >
                          Grant
                        </button>
                        <button
                          onClick={() => handleStudentAction(app._id, "reject")}
                          className="flex-1 text-xs bg-red-500 text-white py-1.5 rounded hover:bg-red-600"
                        >
                          Reject
                        </button>
                      </div>
                        ...
                        <td className="px-3 py-2">
                          {app.status === "MinistryPending" && (
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  handleStudentAction(app._id, "grant")
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleStudentAction(app._id, "reject")
                                }
          ...
          {tab === "institutes" && (
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                <h2 className="section-title mb-0">
                  Institute Registration Applications
                </h2>
                <button
                  className="btn-primary text-xs px-4 py-1 w-full sm:w-auto"
                  onClick={loadInstituteApps}
                >
                  Refresh
                </button>
              </div>
              ...
              {loadingInst && (
                <p className="text-sm py-4">Loading applications...</p>
              )}
              {error && (
                <p className="text-sm py-4 text-red-700">Error: {error}</p>
              )}
                ...
                {instApps.map((app) => (
                  <div key={app.id} className="border rounded p-3 bg-white">
                    ...
                    {app.status === "MinistryPending" && (
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={() => handleInstAction(app.id, "approve")}
                          className="flex-1 text-xs bg-green-600 text-white py-1.5 rounded hover:bg-green-700"
                        >
                          Approve
                        </button>
                        <button
                          onClick={() => handleInstAction(app.id, "reject")}
                          className="flex-1 text-xs bg-red-500 text-white py-1.5 rounded hover:bg-red-600"
                        >
                          Reject
                        </button>
                        ...
                        <td className="px-3 py-2">
                          {app.status === "MinistryPending" && (
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  handleInstAction(app.id, "approve")
                                }
                                className="text-xs bg-green-600 text-white px-2 py-1 rounded hover:bg-green-700"
                              >
                                Approve
                              ...
                              <button
                                onClick={() =>
                                  handleInstAction(app.id, "reject")
                                }
                                className="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600"
                              >
                                Reject
            ...
            <div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  {
                    name: "NTSE – National Talent Search",
                    active: true,
                    applicants: 18750,
                  },
                  {
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500">
                        {s.applicants.toLocaleString()} applicants
                      </p>
                    </div>
                    <span
                      className={`text-xs px-3 py-1 rounded-full font-medium flex-shrink-0 ${s.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
