...
export default function InstituteRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^NSP\d{3}$/.test(form.code))
      return alert(
        "Institute Code must be NSP followed by 3 digits (e.g., NSP001)",
      );
    ...
    try {
      ...
      if (ct.includes("application/json")) {
        ...
        throw new Error(
          `Server returned non-JSON response (status ${res.status}): ${text.slice(0, 400)}`,
        );
      }
      if (!res.ok)
        throw new Error(
      ...
      alert(
        `Success! Institute ID ${data.instId} generated. It will be reviewed by the State Nodal Officer.`,
      );
      setRedirect(true);
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (val.length > 0 && !/^NSP\d{3}$/.test(val)) {
                        setCodeError(
                          "Format: NSP followed by 3 digits (e.g. NSP001)",
                        );
                      } else {
                        setCodeError("");
                      }
                  ...
                  />
                  {codeError && (
                    <div className="text-xs text-red-600 mt-1">{codeError}</div>
                  )}
                </div>
                <div>
                  <label className="form-label">
                    DISE (District Information System for Education) Code *
                  </label>
                  <input
                    className={`form-input ${diseError ? "border-red-500" : ""}`}
                    maxLength={11}
                    pattern="\d{11}"
                    value={form.dise}
                    onChange={(e) => {
                ...
                <div>
                  ...
                  <select
                    className="form-select"
                    value={form.uniState}
                    onChange={(e) => set("uniState", e.target.value)}
                    required
                  >
                    <option value="">-- Select --</option>
                    {states.map((s) => (
                ...
                <div>
                  <label className="form-label">
                    Affiliated University / Board Name *
                  </label>
                  <input
                    className="form-input"
                    placeholder="e.g. University of Mumbai, CBSE etc."
                    value={form.uniName}
                    onChange={(e) => set("uniName", e.target.value)}
                    required
                  />
                </div>
                <div>
              ...
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-gray-50 border rounded p-3">
                  <span className="text-xs text-gray-600 uppercase tracking-wide">
                    Institute Establishment / Registration Certificate *
                  </span>
                  <label className="cursor-pointer self-start sm:self-auto flex-shrink-0">
                    <input
                      type="file"
                      className="hidden"
                      onChange={(e) => set("estCert", e.target.files[0])}
                    />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-gray-50 border rounded p-3">
                  ...
                  <label className="cursor-pointer self-start sm:self-auto flex-shrink-0">
                    <input
                      type="file"
                      className="hidden"
                      onChange={(e) => set("affCert", e.target.files[0])}
                    />
                    <span className="border border-gray-400 text-xs px-4 py-1.5 rounded hover:bg-gray-100 block text-center">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    className={`form-input ${pincodeError ? "border-red-500" : ""}`}
                    maxLength={6}
                    pattern="\d{6}"
                    value={form.pincode}
                    onChange={(e) => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      set("telephone", e.target.value.replace(/\D/g, ""))
                    }
                  />
                </div>
              </div>
            ...
            <div className="flex items-start gap-3 bg-yellow-50 border border-yellow-200 rounded p-3">
              ...
              <label htmlFor="agree" className="text-xs text-gray-600">
                All the details and documents submitted by us are valid and
                true. If found guilty of submitting invalid documents, we may be
                held responsible for that act by us.
              </label>
            </div>
