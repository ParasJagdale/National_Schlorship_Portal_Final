...
const Badge = ({ status }) => {
  const c =
    status === "Pending"
      ...
      : status === "MinistryPending"
        ? "bg-blue-100 text-blue-700"
        : status === "Approved"
          ? "bg-green-100 text-green-700"
          : "bg-red-100 text-red-700";
  const label = status === "MinistryPending" ? "Forwarded to Ministry" : status;
  return (
...
export default function StateDashboard() {
  ...
  useEffect(() => {
    if (tab === "students") {
      setLoading(true);
      fetch("/api/auth/officer/scholarship-applications", {
        credentials: "include",
      })
        .then((res) => res.json())
    ...
    if (tab === "institutes") {
      ...
      setAuthError(false);
      setError(null);
      fetch("/api/auth/officer/institute-applications", {
        credentials: "include",
      })
        .then(async (res) => {
          if (res.status === 401) {
        ...
        .then((data) => setInstApps(data.apps || []))
        .catch((err) => {
  ...
  const handleStudentAction = async (id, action) => {
    try {
      const res = await fetch(`/api/auth/officer/scholarship/${id}/decision`, {
        ...
        credentials: "include",
        body: JSON.stringify({
          decision: action === "forward" ? "approve" : "reject",
        }),
      });
      if (!res.ok) throw new Error("Action failed");
      alert(
        action === "forward"
          ? "Application forwarded to Ministry."
  ...
  const handleInstAction = async (id, action) => {
    try {
      ...
      setInstApps((prev) =>
        prev.map((a) =>
          a._id === id
            ? {
                ...a,
                status: action === "forward" ? "MinistryPending" : "Rejected",
              }
            : a,
        ),
      );
      alert(
        action === "forward"
          ? "Institute registration forwarded to Ministry."
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                <h2 className="section-title mb-0">
                  Student Applications (Forwarded by Institutes)
                </h2>
                  ...
                  <div key={app._id} className="border rounded p-3 bg-white">
                    <div className="flex justify-between items-start gap-2 mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {app.instituteId?.name || app.instituteCode}
                        </p>
                    ...
                    {app.status === "StatePending" && (
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            handleStudentAction(app._id, "forward")
                          }
            ...
            <div>
              ...
              {authError && (
                <div className="p-3 border rounded bg-yellow-50 text-sm text-yellow-800">
                  You are not signed in as a State Nodal Officer. Please{" "}
                  <a href="/login" className="underline">
                    login
                  </a>{" "}
                  with your officer account to view pending institute
                  registrations.
                </div>
              )}
              {error && (
              ...
              {!loading && !authError && instApps.length === 0 && (
                <p className="text-sm py-4">No pending registrations.</p>
              )}
                ...
                {instApps.map((app) => (
                  <div key={app._id} className="border rounded p-3 bg-white">
                    ...
                    {app.status === "Pending" && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleInstAction(app._id, "forward")}
                          className="flex-1 text-xs bg-green-600 text-white py-1.5 rounded hover:bg-green-700"
                        >
                          Forward
                        </button>
                        <button
                          onClick={() => handleInstAction(app._id, "reject")}
                          className="flex-1 text-xs bg-red-500 text-white py-1.5 rounded hover:bg-red-600"
                        >
                          Reject
                        </button>
                        ...
                        <td className="px-3 py-2">
                          {app.status === "Pending" && (
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  handleInstAction(app._id, "forward")
                                }
                                className="text-xs bg-green-600 text-white px-2 py-1 rounded hover:bg-green-700"
                              >
                                Forward
                              ...
                              <button
                                onClick={() =>
                                  handleInstAction(app._id, "reject")
                                }
                                className="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600"
                              >
                                Reject
